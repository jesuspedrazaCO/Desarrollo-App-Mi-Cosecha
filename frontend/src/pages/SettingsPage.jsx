...
export default function SettingsPage() {
  ...
  const [avatarPreview, setAvatarPreview] = useState(
    user?.avatar ? `${UPLOADS_URL}/uploads/${user.avatar}` : null
  )
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      ...
      setAvatarPreview(user?.avatar ? `${UPLOADS_URL}/uploads/${user.avatar}` : null)
    } finally {
      ...
      <div className="rounded-3xl p-6"
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="relative group cursor-pointer flex-shrink-0" onClick={() => fileInputRef.current?.click()}>
            {avatarPreview ? (
              ...
              <div className="w-20 h-20 rounded-2xl flex items-center justify-center text-white text-3xl font-bold"
                style={{
                  background: 'linear-gradient(135deg, #258a4e, #1a6e3c)',
                  boxShadow: '0 4px 16px rgba(37,138,78,0.45)',
                }}>
          ...
          <div>
            ...
            <p className="text-white/35 text-xs mt-2">Haz clic en la foto para cambiarla · JPG, PNG o WEBP, máx 2MB</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-3xl p-6"
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="border-t border-white/10 pt-4">
            <h4 className="text-[13px] font-bold text-white/55 mb-3">Cambiar contraseña (opcional)</h4>
            <Input
      ...
      <div className="rounded-3xl p-6"
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={item.label} className="flex items-center justify-between py-2 border-b border-white/08 last:border-0">
              <span className="text-sm text-white/50">{item.label}</span>
              <span className={`text-sm font-semibold ${item.green ? 'text-emerald-400' : 'text-white/85'}`}>{item.value}</span>
            </div>
          ))}
        </div>
      </div>
