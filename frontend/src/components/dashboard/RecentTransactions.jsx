  ...
  return (
    <div className="bg-white/70 backdrop-blur-xl rounded-3xl shadow-card border border-white/60">
      <div className="px-5 py-4 border-b border-stone-100 flex items-center gap-2">
        <CreditCard size={18} className="text-stone-500" strokeWidth={2} />
        <h3 className="section-title">Transacciones recientes</h3>
      </div>
      <div className="divide-y divide-stone-50">
        {transactions.length === 0 ? (
          <EmptyState icon={<CreditCard size={32} strokeWidth={1.75} />} title="Sin transacciones" description="Registra gastos e ingresos para verlos aquí" />
        ) : (
